...
export const isAdminAuthanticated = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.user.role !== 'Admin') {
    return next(
      new ErrorHandler(
        `${req.user.role} not autharized for this resources!`,
        403
      )
    );
  }
...
export const isPatientAuthanticated = catchAsyncErrors(
  async (req, res, next) => {
...
export const isDoctorAuthanticated = catchAsyncErrors(
  async (req, res, next) => {
...
export const isPatientOrDoctorAuthenticated = catchAsyncErrors(
  async (req, res, next) => {
    ...
    if (patientToken) {
      ...
      if (user.role !== 'Patient')
        return next(
          new ErrorHandler('User not authorized for this resource!', 403)
        );
      req.user = user;
      return next();
    }
