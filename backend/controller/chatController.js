...
export const createChatRoom = catchAsyncErrors(async (req, res, next) => {
  res
    .status(200)
    .json({ success: true, message: 'Handled by socket or message creation.' });
});
...
export const getChatRoomMessages = catchAsyncErrors(async (req, res, next) => {
  ...
  const chats = await Chat.find({ chatRoomId: req.params.chatRoomId }).sort({
    sentAt: 1,
  });
...
export const getUnreadChatsForUser = catchAsyncErrors(
  async (req, res, next) => {
    const chats = await Chat.find({
...
export const uploadChatImage = catchAsyncErrors(async (req, res, next) => {
  ...
  if (imageFile.size > 5 * 1024 * 1024) {
    return res
      .status(400)
      .json({ success: false, message: 'File size exceeds limit of 5MB.' });
  }
